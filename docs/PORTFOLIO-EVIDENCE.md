...
`docs/assets/` is regenerated by the `Portfolio screenshots` workflow against the exact release tag, so the
images always match what reviewers see in production.
