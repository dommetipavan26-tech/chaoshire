...
- 185 automated tests, 97.3% package coverage, and fairness policy enforcement in GitHub Actions. The figures are derived by `scripts/check_build_info.py` from a real test run, so the documents cannot outlive the code they describe.
...
- Implemented configurable decision-CSV ingestion, privacy-conscious aggregate persistence, native PDF/offline HTML reports, security controls, and 185 tests at 97.3% coverage with a CI job that fails when any quoted figure stops matching a real test run.
